...
企业抵税估算与教育优惠互斥，只允许使用官方零售价或自定义企业采购金额。增值税税率固定为 13%，企业所得税估算税率默认为 5%。出于审慎考虑，发票类型和企业身份默认均为“不确定”；只有明确选择“增值税专用发票 + 一般纳税人”时，才会把增值税税额计入预计抵扣。

设备组合区也可发起企业抵税估算，计算器会自动使用组合内设备的官网合计，不会使用组合教育价。
