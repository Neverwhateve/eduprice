...
EDU Quote 是一个移动优先的 Apple 教育优惠报价 PWA，面向需要在 iPhone 上快速准备、展示、保存和复用报价的零售人员。数据来自 `EDU Price.xlsx`，当前网页数据已整理到 `app.js` 中。

项目不依赖前端框架或运行时后端：

- `app.js`：产品目录、移动端产品选型、报价展示、历史记录与已存报价；
- `tax-utils.js`：金额校验、统一格式化与税务纯函数；
- `tax-calculator.js`：旧版税务计算器交互实现（当前报价界面使用 `tax-utils.js`）；
- `sw.js`：离线缓存与版本更新；
...
## Business Purchase 估算

Business Purchase 与教育优惠报价分开呈现，只使用官方零售价进行理论估算。当前移动端界面以固定 13% 增值税、5% 企业所得税率和可抵扣增值税资格作为示例，结果仅供参考。

界面中的价格、合计、节省和税务估算金额统一按元四舍五入显示，不展示小数；底层仍按分计算，避免提前取整造成累计误差。
